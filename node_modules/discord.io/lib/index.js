...
DCP.joinVoiceChannel = function(channelID, callback) {
	var serverID, token, session, endpoint, init, opts, client = this;
	...
	client._ws.on('message', handleWSVoiceMessage.bind(client, client._vChannels[channelID]));
	send(client._ws, init);
};
DCP.leaveVoiceChannel = function(channelID, callback) {
	if (!this._vChannels[channelID]) return console.log("Not in the voice channel: " + channelID);
...
function APIRequest(method, url) {
	...
	var req = requesters[opts.protocol.slice(0, -1)].request(opts, function(res) {
		...
		res.once('end', function() {
			...
			res.body = chunks;
			try {res.body = zlib.gunzipSync(res.body).toString();} catch(e) {}
...
function handleWSClose(code, data) {
	...
	client._ws.removeListener('message', handleWSMessage);
	client._ws = null;
...
function handleWSVoiceMessage(voiceSession, data) {
	...
	if (data.t === "VOICE_STATE_UPDATE") {
		...
		this._ws.removeListener('message', handleWSVoiceMessage);
	}
}
...
function handlevWSClose(voiceSession) {
	...
	return voiceSession.ws.connection.removeListener('message', handlevWSMessage);
}
...
function handleVoiceChannelChange(client, voiceSession) {
	...
	client._ws.once('message', function(m, f) {
		m = f.binary ? JSON.parse(zlib.inflateSync(m).toString()) : JSON.parse(m);
		if (
