...
function DiscordClient(options) {
	...
	self.acceptInvite = function(inviteCode, callback) {
		checkRS(function() {
			if (self.bot) {
				console.log("This account is a 'bot' type account, and cannot use 'acceptInvite'. Please use the inviteLink property instead.")
				return handleErrCB("This account is a 'bot' type account, and cannot use 'acceptInvite'. Please use the inviteLink property instead.", callback);
			}
				...
				for (var key in input) {
					...
					if (key === 'permissions') {
						for (var perm in input[key]) {
							role.permission_values[perm] = input[key][perm];
	...
	function startConnection(gateway) {
		...
		self.getOauthInfo(function(err, res) {
			...
			self.inviteLink = "https://discordapp.com/oauth2/authorize?&client_id=" + res.id + "&scope=bot"
		});
	...
	function handleWSOpen() {
		...
		if (options.shard instanceof Array &&
	...
	function handleWSMessage(data, flags) {
		...
		switch (message.t) {
			...
			case "CHANNEL_UPDATE":
				...
				old = self.servers[message.d.guild_id].channels[message.d.id];
				for (chItem in message.d) {
					if (['guild_id', 'is_private'].indexOf(chItem) === -1) {
						self.servers[message.d.guild_id].channels[message.d.id][chItem] = message.d[chItem];
					}
	...
	/*Utils*/
	function type(n) {
		return Object.prototype.toString.call(n).match(/ (.*)]/)[1].toLowerCase();
	}
	...
	function Role(data) {
		...
		Object.defineProperties(this.permission_values = {}, {
			GENERAL_CREATE_INSTANT_INVITE:{ get: getPerm(0), set: setPerm(0) },
			GENERAL_KICK_MEMBERS:{ get: getPerm(1), set: setPerm(1) },
			GENERAL_BAN_MEMBERS:{ get: getPerm(2), set: setPerm(2) },
			GENERAL_MANAGE_ROLES:{ get: getPerm(3), set: setPerm(3) },
			GENERAL_MANAGE_CHANNELS:{ get: getPerm(4), set: setPerm(4) },
			GENERAL_MANAGE_GUILD:{ get: getPerm(5), set: setPerm(5) },

			TEXT_READ_MESSAGES:{ get: getPerm(10), set: setPerm(10) },
			TEXT_SEND_MESSAGES:{ get: getPerm(11), set: setPerm(11) },
			TEXT_SEND_TTS_MESSAGE:{ get: getPerm(12), set: setPerm(12) },
			TEXT_MANAGE_MESSAGES:{ get: getPerm(13), set: setPerm(13) },
			TEXT_EMBED_LINKS:{ get: getPerm(14), set: setPerm(14) },
			TEXT_ATTACH_FILES:{ get: getPerm(15), set: setPerm(15) },
			TEXT_READ_MESSAGE_HISTORY:{ get: getPerm(16), set: setPerm(16) },
			TEXT_MENTION_EVERYONE:{ get: getPerm(17), set: setPerm(17) },

			VOICE_CONNECT:{ get: getPerm(20), set: setPerm(20) },
			VOICE_SPEAK:{ get: getPerm(21), set: setPerm(21) },
			VOICE_MUTE_MEMBERS:{ get: getPerm(22), set: setPerm(22) },
			VOICE_DEAFEN_MEMBERS:{ get: getPerm(23), set: setPerm(23) },
			VOICE_MOVE_MEMBERS:{ get: getPerm(24), set: setPerm(24) },
			VOICE_USE_VAD:{ get: getPerm(25), set: setPerm(25) },
		});
		...
		function setPerm(bit) {
			return function(v) {
				if (v === true) return self.permissions |= (1 << (bit));
